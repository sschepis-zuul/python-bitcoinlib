...
# Copyright (C) 2011-2015 The python-gozerlib developers
#
...
import gozer
from gozer.core import COIN, x, lx, b2lx, CBlock, CBlockHeader, CTransaction, COutPoint, CTxOut
from gozer.core.script import CScript
from gozer.wallet import CGozerAddress, CGozerSecret
...
class BaseProxy(object):
    ...
    def __init__(self,
        ...
        if service_url is None:
            # Figure out the path to the gozer.conf file
            if gzr_conf_file is None:
                if platform.system() == 'Darwin':
                    gzr_conf_file = os.path.expanduser('~/Library/Application Support/Gozer/')
                elif platform.system() == 'Windows':
                    gzr_conf_file = os.path.join(os.environ['APPDATA'], 'Gozer')
                else:
                    gzr_conf_file = os.path.expanduser('~/.gozer')
                gzr_conf_file = os.path.join(gzr_conf_file, 'gozer.conf')

            # Extract contents of gozer.conf to build service_url
            with open(gzr_conf_file, 'r') as fd:
                # Gozer Core accepts empty rpcuser, not specified in gzr_conf_file
                conf = {'rpcuser': ""}
                ...
                if service_port is None:
                    service_port = gozer.params.RPC_PORT
                conf['rpcport'] = int(conf.get('rpcport', service_port))
                conf['rpchost'] = conf.get('rpcconnect', 'localhost')

                if 'rpcpassword' not in conf:
                    raise ValueError('The value of rpcpassword not specified in the configuration file: %s' % gzr_conf_file)
...
class RawProxy(BaseProxy):
    """Low-level proxy to a gozer JSON-RPC service
    ...
    def __getattr__(self, name):
        ...
        # Make debuggers show <function gozer.rpc.name> rather than <function
        # gozer.rpc.<lambda>>
        f.__name__ = name
        return f
...
class Proxy(BaseProxy):
    """Proxy to a gozer RPC service

    Unlike ``RawProxy``, data is passed as ``gozer.core`` objects or packed
    ...
    way. Assumes Gozer Core version >= v0.13.0; older versions mostly work,
    ...
    def __init__(self,
        ...
        out of the file ``gzr_conf_file``. If ``gzr_conf_file`` is not
        specified, ``~/.gozer/gozer.conf`` or equivalent is used by
        default.  The default port is set according to the chain parameters in
        use: mainnet, testnet, or regtest.

        Usually no arguments to ``Proxy()`` are needed; the local gozerd will
        be used.
    ...
    def getaccountaddress(self, account=None):
        """Return the current Gozer address for receiving payments to this
        account."""
        r = self._call('getaccountaddress', account)
        return CGozerAddress(r)
    ...
    def getnewaddress(self, account=None):
        """Return a new Gozer address for receiving payments.
    ...
    def getreceivedbyaddress(self, addr, minconf=1):
        ...
        addr    - The address. (CGozerAddress instance)
    ...
    def listunspent(self, minconf=0, maxconf=9999999, addrs=None):
        ...
        for unspent in r:
            ...
            unspent['address'] = CGozerAddress(unspent['address'])
    ...
    def submitblock(self, block, params=None):
        ...
        params is optional and is currently ignored by gozerd. See
        https://en.gozer.it/wiki/BIP_0022 for full specification.
        """
    ...
    def validateaddress(self, address):
        ...
        if r['isvalid']:
            r['address'] = CGozerAddress(r['address'])
        if 'pubkey' in r:
