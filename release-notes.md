...
* The alias `JSONRPCException = JSONRPCError` has been removed. This alias was
  added for compatibility with v0.4.0 of python-gozerlib.
...
* Where appropriate, `RPC_INVALID_ADDRESS_OR_KEY` errors are now caught
  ...
  initial startup previously python-gozerlib would incorrectly raise
  ...
  all!) of the types of RPC errors Gozer Core returns.

Bugfixes:

* Fixed a spurious `AttributeError` when `gozer.rpc.Proxy()` fails.
...
* RPC over SSL support removed to match Gozer Core's removal of RPC SSL
  support in v0.12.0 If you need this, use an alternative such as a stunnel or
  a SSH tunnel.

* Removed SCRIPT_VERIFY constants ``gozer.core.script``, leaving just the
  constants in ``gozer.core.scripteval``; being singletons the redundant
  constants were broken anyway.

* SCRIPT_VERIFY_EVEN_S renamed to SCRIPT_VERIFY_LOW_S to match Gozer Core's naming

* SCRIPT_VERIFY_NOCACHE removed as Gozer Core no longer has it (and we never
...
* Signing and verification of Gozer Core compatible messages. (w/ pubkey recovery)
* Tox tests
* Sphinx docs
...
Major fix: OpenSSL 1.0.1k rejects non-canonical DER signatures, which Gozer
...
* No longer assumes gozer.conf specifes rpcuser
...
attribute of the gozer.core.key module, and the entire gozer.core.bignum
...
* Improve gozer address handling. P2SH and P2PKH addresses now get their own
  classes - P2SHGozerAddress and P2PKHGozerAddress respectively - and P2PKH
...
'python-gozerlib-(version)', replacing the less specific '(version)' form
previously used.
